...
if [ "$VERBOSE" = true ]; then
    if docker build -t azure-devops-mcp:local .; then
    ...
    if docker build -t azure-devops-mcp:local . > "$DOCKER_LOG" 2>&1; then
...
echo -e "\n${GREEN}Build complete!${NC} Image tagged as azure-devops-mcp:local"
